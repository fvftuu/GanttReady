...
export function buildNetworkSvg(params: any): string {
    ...
    let dw = p.dayWidth || 8;
    let tsm = (window as any)._netTimeScaleMode || 0;
    ...
    if (p.timeParams && p.timeParams.activities) {
        let allSegs = collectAllSegments(p.layout, p.timeParams.activities,
            p.timeParams.relations || [], isTimeMode, nr);
        let crossParts: string[] = [];
    ...
    var rulerBottom = (window as any)._netRulerBottom || (cvH - 10);
    ...
    (window as any)._netSvgHeight = finalH;

    // 清空拖拽偏移（已应用到新 SVG）
    (window as any)._netPendingOffsets = {};
...
// ---- 行背景 ----
function renderRowBg(parent: SVGElement, eventsMap: any, cvW: number, layerH: number, mode: string): void {
    let ySet: { [y: number]: boolean } = {};
    Object.keys(eventsMap).forEach(function(eid) {
        let evt = eventsMap[eid];
        if (evt.y !== undefined) ySet[evt.y] = true;
    });
...
// 竖直线: 日期对齐线 (每天一条), 范围同上
function renderGrid(parent: SVGElement,
    totalDays: number, dw: number,
    showH: boolean, showV: boolean,
    layerH: number, marginTop: number, maxY: number, ML: number): void {
...
function renderTimeline(parent: SVGElement, sd: Date, dw: number,
    totalDays: number, isTimeMode: boolean, tsm: number): void {
...
// ---- 活动箭线 ----
function renderActivities(parent: SVGElement, activities: any[], eventsMap: any,
    showCritical: boolean, showFloat: boolean,
    isTimeMode: boolean, dw: number, sd: Date,
    nr: number, p: any): void {
    let critActs: any[] = [];
    let nonCritActs: any[] = [];
    ...
    var inDegActs: Record<string, any[]> = {};
    (activities || []).forEach(function(act: any) {
        var tgt = act.target;
        if (tgt) {
    ...
    Object.keys(inDegActs).forEach(function(tgt) {
        inDegActs[tgt].sort(function(a: any, b: any) { return a.id - b.id; });
    });

    [nonCritActs, critActs].forEach(function(actList: any[]) {
        actList.forEach(function(act: any) {
            ...
            var inDegIdx = 0;
            tgtActs.forEach(function(a: any, i: number) { if (a.id === act.id) inDegIdx = i; });
            var tgtOffY = 0;
            if (inDegTgt > 1) {
                var gap = 6;
...
// ---- 母线法 ----
function renderBusbar(parent: SVGElement, activities: any[], eventsMap: any, nr: number): void {
    let hasBus = false;
    const g = svgEl('g', { class: 'busbar', opacity: 0.5 });

    Object.keys(eventsMap).forEach(function(eid: string) {
        let evt = eventsMap[eid];
        if (evt.isVirtual) return;
        let ex = evt.x || 0, ey = evt.y || 0;

        // 出度母线（右侧），出度 >= 2 使用母线
        let outActs = activities.filter(function(a: any) { return a.source === eid; });
        if (outActs.length >= 2) {
            hasBus = true;
            let tys = outActs.map(function(a: any) {
                let t = eventsMap[a.target]; return t ? (t.y || 0) : ey;
            });
            ...
            outActs.forEach(function(a: any) {
                let t = eventsMap[a.target]; if (!t) return;
        ...
        // 入度母线（左侧），入度 >= 2 使用母线
        let inActs = activities.filter(function(a: any) { return a.target === eid; });
        if (inActs.length >= 2) {
            hasBus = true;
            let sys = inActs.map(function(a: any) {
                let s = eventsMap[a.source]; return s ? (s.y || 0) : ey;
            });
            ...
            inActs.forEach(function(a: any) {
                let s = eventsMap[a.source]; if (!s) return;
...
// ---- 事件节点 ----
function renderNodes(parent: SVGElement, eventsMap: any, showCritical: boolean,
    showFloat: boolean, isTimeMode: boolean,
    nr: number, nfs: number): void {
...
// ---- 今日线 ----
function renderTodayLine(parent: SVGElement, p: any, sd: Date, dw: number,
    cvH: number, isTimeMode: boolean): void {
...
// ---- 前锋线 ----
function renderProgressLine(parent: SVGElement, p: any, sd: Date, dw: number,
    cvH: number, eventsMap: any, isTimeMode: boolean): void {
...
// ---- 进度曲线 ----
function renderProgressCurve(parent: SVGElement, p: any, sd: Date, dw: number,
    isTimeMode: boolean): void {
    ...
    for (let d = 0; d < p.totalDays; d++) {
        ...
        acts.forEach(function(a: any) {
            let ae = a.es || 0;
            let af = a.ef || 0;
            if (d >= ae && d < af) { dc += (a.completion || 0); dn++; }
        });
...
// ---- 查找范围 ----
function findExtents(eventsMap: any, totalDays: number,
    dw: number, nr: number): { lastRowY: number; rightmostX: number } {
...
// ---- 图例 ----
function renderLegend(parent: SVGElement, lastRowY: number, layerH: number,
    totalDuration: number): void {
    let legendTop = lastRowY + layerH + 10;
...
// ---- 底部标尺（翻转：日期在上层，月份在下层） ----
function renderBottomRuler(parent: SVGElement, p: any, sd: Date, dw: number,
    cvW: number, cvH: number, isTimeMode: boolean): void {
    if (!isTimeMode) return;
    ...
    // 记录底部标尺底部位置，供自适应高度使用
    (window as any)._netRulerBottom = rulerY + UPPER_H + LOWER_H;
}