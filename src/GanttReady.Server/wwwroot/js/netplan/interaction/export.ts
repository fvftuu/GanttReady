...
export function downloadSvg(filename: string = 'network.svg'): void {
  const svg = getSvg();
  downloadSVGFile(svg, filename);
}

export function exportPng(filename: string = 'network.png'): void {
  const svg = getSvg();
  exportSvgAsPng(svg, filename);
}

export function printSvg(printZoomVal: string = '100%'): void {
  const svg = getSvg();
  if (!svg) return;
