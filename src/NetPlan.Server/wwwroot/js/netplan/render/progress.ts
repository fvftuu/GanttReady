...
export function renderProgressLine(
  ...
  if (!opts.showProgressLine) return;
  ...
  const todayOffset = Math.round(
    (today.getTime() - projectStartDate.getTime()) / 86400000
  );
  ...
  if (opts.progressDate) {
    ...
    const progressOffset = Math.round(
      (savedDate.getTime() - projectStartDate.getTime()) / 86400000
    );
  ...
  if (opts.progressDate) {
    for (const evt of events) {
      if (evt.isVirtual) continue;
      const progDate = new Date(opts.progressDate);
      const progOff = Math.round(
        (progDate.getTime() - projectStartDate.getTime()) / 86400000
      );
...
export function updateProgressColors(
  events: EventNode[],
  progressCheckDate: Date | null,
