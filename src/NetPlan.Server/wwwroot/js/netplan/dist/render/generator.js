...
export function buildNetworkSvg(params) {
    ...
    var crossArcs = [];
    function emitAct() { if (!actGroupOpen) {
    ...
    (activities || []).forEach(function (act) { (act.isCritical && showCritical ? critActs : nonCritActs).push(act); });
    // 工作编号: 非关键+虚工作先
    [nonCritActs, critActs].forEach(function (actList) {
        actList.forEach(function (act) {
            ...
            // 收集交叉弧信息
            if (isDummy)
                crossArcs.push({ x1: x1, y1: y1, x2: x2, y2: y2, type: 'dummy' });
        });
    });
    if (actGroupOpen)
        parts.push('</g>');
...
function renderRowBackground(p) {
    ...
    Object.keys(events).forEach(function (eid) {
        var evt = events[eid];
        if (evt.y !== undefined && layers.indexOf(evt.y) === -1)
            layers.push(evt.y);
    });
